...
df2 = df2.filter((df2.gps_diff < 50) & (df2.time_norm >= '1800-01-01') & (df2.actor == 'David Chilton Phillips'))

df2.repartition(1).select('e_id', 'actor', 'time_norm', 'gps_diff').write.format('com.databricks.spark.csv') \
    .option('sep', ';').option('header', True).save('ExactLocationAndActor1')
