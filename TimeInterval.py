...
df = df.filter((size(df.loc) > 0) & (df.time_norm >= '1941-06-22') & (df.time_norm <= '1945-05-09')).cache()
df = df.orderBy("time_norm").cache()

df.repartition(1).select('e_id', 'actor', 'geo_name', 'time_norm').write.format('com.databricks.spark.csv') \
    .option('sep', ';').option('header', True).save('TimeInterval1')

#print(df2.select('e_id', 'actor', 'time_norm', 'gps_diff').collect())
